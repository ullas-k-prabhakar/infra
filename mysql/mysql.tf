...
resource "google_compute_instance" "mysql-plain-server" {
  boot_disk {
    auto_delete = true
    device_name = "mysql-plain-server"
  ...
  metadata = {
    ...
    startup-script = templatefile("${path.module}/utils/setup-mariadb.tpl", {
