...
resource "google_compute_instance" "mqtt-plain-server" {
  boot_disk {
    auto_delete = true
    device_name = "mqtt-plain-server"
  ...
  metadata = {
    ...
    startup-script = templatefile("${path.module}/utils/install_mosquitto.tpl", {
